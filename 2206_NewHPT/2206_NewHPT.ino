...
//  M5.Lcd.printf("Date:%04d.%02d.%02d %s\n", RTC_DateStruct.Year, RTC_DateStruct.Month, RTC_DateStruct.Date, wd[RTC_DateStruct.WeekDay]);
  ...
  M5.Lcd.drawRect(95, 63, 55, 15, GREEN);
  M5.Lcd.setCursor(105, 66, 1);
  M5.Lcd.setTextColor(GREEN);
  M5.Lcd.println("AP_HPT");
...
//  pinMode(input1,INPUT_PULLUP);//36
  pinMode(input2,OUTPUT);    //26 
  pinMode(input3,INPUT_PULLUP);    //0
...
void loop() {
  ...
  //  if(chkflag){
        //
  //  }else{
  int before_sw1=digitalRead(input1);     //G36
  int before_sw2=digitalRead(input3);     //G0
...
//  }else{
    ...
    if (before_sw1==0){      //cdsセンサーの値読み取りを逆転させる（0<=>1) //36を使用する場合は、3.3v直接入力となり、不要
        before_sw1_gyaku=1;
...
//    Serial.println(time(NULL));
    delay(10);
    int sw1=digitalRead(input1); //G36
    int sw2=digitalRead(input3); //G0
...
//  }else{  
    ...
    if (sw1==0){      //cdsセンサーの値読み取りを逆転させる（0<=>1)　//36を使用する場合は、3.3v直接入力となり、不要
        sw1_gyaku=1;
...
//  Serial.print("SWのgyaku状態,G26 & G0");
//  Serial.print("-");
//  Serial.print(before_sw);
//  Serial.print("-");
//  Serial.println(sw);
//  }
   //チャタリング対策
...
//          Serial.println("swChangeに入りました");
            ...
            switch (sw){
              case 0:
//                    if(nowMillis>500){       //１秒以上点灯している状態（寸動を除去する）    
                        switch(beforeinput){
                          case 0:
//                                delay(10);
//                                
...
//                                delay(10);
//                                sendToFirebase(MachineNo1,"RUN1"); 
                                break;
...
//                     if(nowMillis>500){       //１秒以上点灯している状態（寸動を除去する）         
                     ...
                     break;
                        break;
               case 2://フレーム起動
//                     if(nowMillis>500){       //1秒以上段取りSWがオンになっている状態（寸動や軽微なエラーを除去する）
                     switch(beforeinput){
                          case 0:
                               delay(10);
                               sendToFirebase(MachineNo2,"RUN1");
                               break;
                          case 1:
                                delay(10);
                                sendToFirebase(MachineNo2,"RUN1");
...
//    Firebase.setBool(firebaseData, is_power_on_key,false);  //Firebaseに、falseを代入
  ...
  if(digitalRead(R_switch)==LOW){
    ...
    if(cur_value1==cur_value2){ //20220707　２つ目を2 に変更した
      RunBool1=false;
    }
  }
  delay(10);
